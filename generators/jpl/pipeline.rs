...
//!
//! Canonical generator source moved from `siderust/scripts/jpl/pipeline.rs`.
...
fn generate_stub_rust_module(cfg: &DeConfig, path: &Path) -> anyhow::Result<()> {
    ...
    writeln!(f, "/// Pre-built segment descriptors for the {} bodies.", cfg.label)?;
    writeln!(f)?;
...
fn generate_rust_module(
    ...
    for (name, meta) in bodies {
        ...
        writeln!(f, "        debug_assert!(i < N_RECORDS, \"{} {}: record index {{}} out of range (max {{}})\", i, N_RECORDS - 1);", cfg.label, upper)?;
    ...
    writeln!(f, "}};")?;
    writeln!(f)?;
    writeln!(f, "/// Segment descriptor for the Earth-Moon Barycenter (NAIF 3 → SSB).")?;
    writeln!(f, "pub const EMB: SegmentDescriptor = SegmentDescriptor {{")?;
    writeln!(f, "    init: crate::qtty::Seconds::new(emb::INIT),")?;
    writeln!(f, "    intlen: crate::qtty::Seconds::new(emb::INTLEN),")?;
    writeln!(f, "    ncoeff: emb::NCOEFF,")?;
...
fn ensure_bsp(cfg: &DeConfig, data_dir: &Path) -> anyhow::Result<std::path::PathBuf> {
    ...
    // Download from NAIF
    eprintln!("  Downloading {} from {}", cfg.label, cfg.bsp_url);
    let mut cmd = if command_exists("curl") {
        let mut c = Command::new("curl");
        c.args(["-fSL", "--retry", "3", "--retry-delay", "2", "-o"]);
        c.arg(&bsp_path);
        c.arg(cfg.bsp_url);
        c
    } else if command_exists("wget") {
        let mut w = Command::new("wget");
        w.args(["-q", "--tries=3", "--waitretry=2", "-O"]);
        w.arg(&bsp_path);
        w.arg(cfg.bsp_url);
        w
    } else {
        anyhow::bail!("No downloader available (need curl or wget)");
    };

    let status = cmd.status()?;
    if !status.success() {
        anyhow::bail!("Download failed with status {:?}", status.code());
    }

    let meta = std::fs::metadata(&bsp_path)?;
    if meta.len() < cfg.min_bsp_size {
        anyhow::bail!("Downloaded BSP too small: {} bytes", meta.len());
    }
    Ok(bsp_path)
}
