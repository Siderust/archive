...
//!    file SHA-256.
//! 5. Exits non-zero if anything fails so CI can react.
//!
//! It does **not** regenerate any Rust source code. Downstream crates (e.g.
//! tempoch) consume the raw files at runtime via `siderust-archive` itself.
//!
//! Usage
//! -----
//! ```text
...
//! ```
//!
//! The default `--archive-root` is the parent of the `CARGO_MANIFEST_DIR`
//! grandparent, which makes the tool work out of the box when run from inside
//! the archive workspace (`cargo run -p siderust-archive --bin
//! siderust-archive-update-time-data --features fetch`).
        ...
        "siderust-archive-update-time-data\n\
         ...
         OPTIONS:\n\
             --archive-root <PATH>   Path to the archive repository root.\n\
                                     Defaults to the archive workspace if run from\n\
                                     inside it, otherwise the current directory.\n\
...
fn default_archive_root() -> PathBuf {
    // CARGO_MANIFEST_DIR = <archive>/crates/siderust-archive.
    // Two `parent()` calls take us to <archive>.
...
fn run(args: &CliArgs) -> Result<String, String> {
    ...
    // Stage the download in a separate temporary directory inside the archive
    // root so the swap is on the same filesystem and therefore atomic.
    ...
    // TimeDataManager writes its cached bundle under `<staging>/bundle/`.
    ...
    // Provenance JSON: copy whether or not the data changed so the file always
    // exists; downstream tooling treats its presence as a freshness signal.
    ...
    // Clean staging only after the swap succeeds.
