...
//! Parameters (EOP / finals2000A.all).
//!
//! This module is the canonical, repo-agnostic home for the IERS download,
//! parsing, provenance, and integrity logic. It is extracted verbatim from
//! runtime via [`TimeDataManager`] (behind the `fetch` feature) which downloads
//! the raw upstream files, verifies their SHA-256, and caches them with a
...
pub use refs::{
    DELTA_T_OBSERVED_URL, DELTA_T_PREDICTIONS_URL, EOP_FINALS_URL, PRE_1961_TAI_MINUS_UTC_APPROX,
    TEMPOCH_DATA_DIR_ENV, UTC_TAI_HISTORY_URL,
};
...
#[cfg(feature = "bundled-time")]
pub mod bundled;

#[cfg(feature = "bundled-time")]
pub use bundled::bundled_time_data;

/// Provenance for the IERS time-scale datasets managed by this module.
///
/// Returns static metadata describing the upstream sources.  For a provenance
/// record that includes fetch timestamps and per-file SHA-256 hashes of a
/// specific download, use [`TimeDataProvenance`] returned by
/// [`TimeDataManager::load`].
pub fn provenance() -> crate::provenance::DatasetProvenance {
    crate::provenance::DatasetProvenance {
        source: "NAIF Leap Second Kernel naif0012.tls; \
                 IERS finals2000A.all (EOP); USNO deltat.data / deltat.preds; \
                 IERS UTC-TAI.history"
            .to_string(),
        generator: "siderust-archive/src/time/mod.rs (runtime fetch via TimeDataManager)"
            .to_string(),
