...
pub(crate) fn run_regen(raw_dir: &Path, out_dir: &Path) -> Result<()> {
    ...
    anyhow::ensure!(ls_a.len() == 678, "nut00a_ls: expected 678 rows, got {}", ls_a.len());
    anyhow::ensure!(pl_a.len() == 687, "nut00a_pl: expected 687 rows, got {}", pl_a.len());
    anyhow::ensure!(ls_b.len() == 77,  "nut00b_ls: expected 77 rows, got {}",  ls_b.len());

    let n_ls_b = ls_b.len();

    let mut out = String::new();
    writeln!(out, "// AUTOGENERATED by siderust-archive build.rs — DO NOT EDIT BY HAND")?;
    writeln!(out, "// Source: src/nutation/raw/nut00a_ls.csv, nut00a_pl.csv, nut00b_ls.csv")?;
    writeln!(out)?;

    // NUT00A_LS — [[f64; 11]; 678]
    writeln!(out, "/// IAU 2000A luni-solar nutation series (678 terms).")?;
    writeln!(out, "/// Columns: nl, nlp, nf, nd, nom, sp, spt, cp, ce, cet, se")?;
    ...
    // NUT00B_LS — [[f64; 11]; 77]
    writeln!(out, "/// IAU 2000B luni-solar nutation series ({n_ls_b} terms).")?;
    writeln!(out, "/// Columns: nl, nlp, nf, nd, nom, sp, spt, cp, ce, cet, se")?;
    writeln!(out, "#[rustfmt::skip]")?;
